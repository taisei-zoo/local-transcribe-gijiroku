...
V3は、1台のWindows端末上で、音声文字起こし・文書テキスト抽出・ローカルLLM要約をGUIから実行する単体利用版です。

共有フォルダーを巡回して複数人の依頼を処理するV4とは別物です。V3には `_UPLOAD_DONE.txt`、`server.lock`、処理中/完了/失敗フォルダー管理、タスクスケジューラ前提の処理は含めていません。
...
V3はGUIで警告や処理状況を確認できるため、V4で使っていた以下のサーバー運用確認用ファイルは標準出力しません。
